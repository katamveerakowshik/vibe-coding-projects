...
const SubscriptionCard: React.FC<Props> = ({ service, activeCount, onClick }) => {
  return (
    <div 
      onClick={onClick}
      ...
      style={{ '--pulse-color': service.color } as React.CSSProperties}
    >
      ...
      <div className="absolute top-0 left-0 w-1 h-full opacity-80" style={{ backgroundColor: service.color }} />

      <div className="p-6 flex flex-col h-40 justify-between relative z-10">
        <div className="flex justify-between items-start">
          <BrandLogo service={service.name} color={service.color} />
        ...
        <div>
          ...
          <p className="text-white/40 text-sm">Join existing nodes</p>
        </div>
      </div>
